...
{
	int i;
	for (i = 0; i < sizeof(aw_private.pm.clk)/sizeof(aw_private.pm.clk[0]);
		 i++) {
		if (aw_private.pm.clk[i].clk_handle) {
			if (aw_private.pm.clk[i].parent_clk_num >= 0)
				if (clk_set_parent(
					aw_private.pm.clk[i].clk_handle,
					aw_private.pm.clk[aw_private.pm.clk[i]
						.parent_clk_num].clk_handle))
					MALI_PRINT_ERROR((
			"Failed to set theparent of the clock %s to %s!\n",
						aw_private.pm.clk[i].clk_name,
						aw_private.pm.clk[aw_private.pm
						.clk[i].parent_clk_num].
						clk_name));
		} else {
...
static void clk_reset_wrap(void)
{
	...
	for (i = 0; i < sizeof(aw_private.pm.clk)/sizeof(aw_private.pm.clk[0]);
		...
		if (aw_private.pm.clk[i].need_reset) {
			if (clk_reset(aw_private.pm.clk[i].clk_handle,
				AW_CCU_CLK_NRESET))
				MALI_PRINT_ERROR((
					"Failed to reset the clock%s!\n",
					aw_private.pm.clk[i].clk_name));
		} else {
			continue;
		}
...
static void aw_init(struct platform_device *device)
{
	...
	sun5i_get_fb_addr_para(&fb_addr_para);
#else
