...
build_kernel()
{
    ...
    if [ "${ARCH}" = "arm" ]; then
		if [ "${LICHEE_CHIP}" = "sun3iw1p1" ]; then
			...
			if [ "x$LICHEE_BUILD_ROOT" == "xbuildroot-201611" ];then
				echo "rootfs_32bit.cpio_buildroot_201611.gz"
				RAMFS_SOURCE=rootfs_32bit.cpio_buildroot_201611.$RAMFS_COMPRESS_METHOD
			else
				echo "rootfs_32bit.cpio.gz"
