...
static struct disp_layer_config aut_config[FB_MAX];
...
int show_fb_to_ui()
{
	...
	s32 layer_id;
	for (fb_id =0;fb_id<FB_MAX;fb_id++){
		s32 chan = g_fbi.layer_hdl[fb_id][0];
		s32 layer_id = g_fbi.layer_hdl[fb_id][1];
			...
			if (mgr && mgr->get_layer_config && mgr->set_layer_config) {
				if(needclear){
					...
					if((chanback == 1)&&(layer_idback==0)){
						config1.info.zorder =3;
					}else if((chanback == 2)&&(layer_idback==0)){
						config1.info.zorder =2;
					}else
					{
						config1.info.zorder =0;
					}
				...
				memcpy(&config,&aut_config[fb_id], sizeof(struct disp_layer_config));
				config.channel = chan;
				config.layer_id = layer_id;
				config.enable =0;
				aut_config[fb_id].enable =0;
				if((chan == 1)&&(layer_id==0)){
					config.info.zorder =3;
					aut_config[fb_id].info.zorder =3;
				}else if((chan == 2)&&(layer_id==0)){
					config.info.zorder =2;
					aut_config[fb_id].info.zorder =2;
				}else
				{
					config.info.zorder =0;
					aut_config[fb_id].info.zorder =0;
				}
				__wrn("sunxi_fb_to_display set fb%d UIid %d chan[%d]lid[%d]z[%d]\r\n",fb_id,UIid,chan,layer_id,config.info.zorder);
				...
				if(((chan == 1)&&(layer_id==0))||((chan == 2)&&(layer_id==0))){
...
static int sunxi_fb_pan_display(struct fb_var_screeninfo *var,struct fb_info *info)
{
	...
	for (sel = 0; sel < num_screens; sel++) {
		if (sel==g_fbi.fb_mode[info->node]) {
			...
			struct disp_layer_config *config=&aut_config[info->node];
			struct disp_manager *mgr = g_disp_drv.mgr[sel];
...
static int sunxi_fb_set_par(struct fb_info *info)
{
	...
	for (sel = 0; sel < num_screens; sel++) {
		if (sel==g_fbi.fb_mode[info->node]) {
			...
			struct disp_layer_config *config=&aut_config[info->node];
			struct disp_manager *mgr = g_disp_drv.mgr[sel];
			//memcpy(config,aut_config[info->node], sizeof(struct disp_layer_config));
...
static s32 display_fb_request(u32 fb_id, struct disp_fb_create_info *fb_para)
{
	...
	//struct disp_layer_config aut_config;
	u32 sel;
	u32 xres, yres;
	u32 num_screens;
	...
	for (sel = 0; sel < num_screens; sel++) {
		if (sel == fb_para->fb_mode)	{
			...
			memset(&aut_config[fb_id], 0, sizeof(struct disp_layer_config));

			aut_config[fb_id].channel = g_fbi.layer_hdl[fb_id][0];
			aut_config[fb_id].layer_id = g_fbi.layer_hdl[fb_id][1];
			if((fb_id ==0)/*||(fb_id ==1)||(fb_id ==2)*/){
				aut_config[fb_id].enable = 1;
			...
			aut_config[fb_id].info.screen_win.width = (0 == fb_para->output_width)? src_width:fb_para->output_width;
			aut_config[fb_id].info.screen_win.height = (0 == fb_para->output_height)? src_width:fb_para->output_height;

			aut_config[fb_id].info.mode = LAYER_MODE_BUFFER;
			if(fb_id ==0){
				aut_config[fb_id].info.zorder =3;
			}else if(fb_id ==1){
				aut_config[fb_id].info.zorder =2;
			...
			printk(KERN_ERR"display_fb_request fb%d zorder %d \r\n",fb_id,aut_config[fb_id].info.zorder);
			aut_config[fb_id].info.alpha_mode = 0;
			aut_config[fb_id].info.alpha_value = 0;
			aut_config[fb_id].info.fb.crop.x = ((long long)0) << 32;
			aut_config[fb_id].info.fb.crop.y = ((long long)y_offset) << 32;
			aut_config[fb_id].info.fb.crop.width = ((long long)src_width) << 32;
			aut_config[fb_id].info.fb.crop.height = ((long long)src_height) << 32;
			aut_config[fb_id].info.screen_win.x = 0;
			aut_config[fb_id].info.screen_win.y = 0;
			var_to_disp_fb(&(aut_config[fb_id].info.fb), &(info->var), &(info->fix));
			aut_config[fb_id].info.fb.addr[0] = (unsigned long long)info->fix.smem_start;
			aut_config[fb_id].info.fb.addr[1] = 0;
			aut_config[fb_id].info.fb.addr[2] = 0;
			aut_config[fb_id].info.fb.flags = DISP_BF_NORMAL;
			aut_config[fb_id].info.fb.scan = DISP_SCAN_PROGRESSIVE;
			aut_config[fb_id].info.fb.size[0].width = fb_para->width;
			aut_config[fb_id].info.fb.size[0].height = fb_para->height;
			aut_config[fb_id].info.fb.size[1].width = fb_para->width;
			aut_config[fb_id].info.fb.size[1].height = fb_para->height;
			aut_config[fb_id].info.fb.size[2].width = fb_para->width;
			aut_config[fb_id].info.fb.size[2].height = fb_para->height;
			aut_config[fb_id].info.fb.color_space = DISP_BT601;

			if (mgr && mgr->set_layer_config)
				mgr->set_layer_config(mgr, &aut_config[fb_id], 1);
		}
	}
...
s32 fb_init(struct platform_device *pdev)
{
	...
	if (g_disp_drv.disp_init.b_init) {
		...
		fb_num = UI_FB_NUM;
		for (i = 0; i<fb_num; i++)	{
			printk("fbinit display_fb_request fb%d \r\n",i);
			...
			disp_fb_to_var(g_disp_drv.disp_init.format[screen_id], &(g_fbi.fbinfo[i]->var));
			fb_para.buffer_num= g_disp_drv.disp_init.buffer_num[screen_id];
			if ((g_disp_drv.disp_init.fb_width[screen_id] == 0) || (g_disp_drv.disp_init.fb_height[screen_id] == 0))	{
				...
				fb_para.width = g_disp_drv.disp_init.fb_width[screen_id];
				fb_para.height = g_disp_drv.disp_init.fb_height[screen_id];
			}
...
s32 fb_exit(void)
{
	...
	for (fb_id=0; fb_id<UI_FB_NUM; fb_id++) {
