...
int disp_al_enhance_apply(unsigned int disp, struct disp_enhance_config *config)
{
	if (config->flags & ENHANCE_MODE_DIRTY) {
		...
		csc_config.brightness = config->info.bright;
		csc_config.contrast= config->info.contrast;
		csc_config.saturation = config->info.saturation;
		csc_config.hue = config->info.hue;
