...
void set_mcu_back(on)
{
      if(on)
      {
	       disp_sys_power_enable("axp22_ldoio0");
	       disp_sys_power_enable("axp22_ldoio1");
      }
      else
      {
	       disp_sys_power_disable("axp22_ldoio0");
	       disp_sys_power_disable("axp22_ldoio1");
      }

}

void get_g_disp_drv_para(int sel,int *br,int *con,int *satu,int *hue)
{
	*br = g_disp_drv.disp_init.brightness[sel];
	*con = g_disp_drv.disp_init.contrast[sel];
	*satu = g_disp_drv.disp_init.satuation[sel];
	*hue = g_disp_drv.disp_init.hue[sel];
	return;
}

EXPORT_SYMBOL(get_g_disp_drv_para);
...
static ssize_t disp_fbl_enable_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
	struct disp_manager *mgr = NULL;
	struct disp_device *dispdev = NULL;
	struct fdisp_lcd_private_data *lcdp=NULL;

	mgr = g_disp_drv.mgr[0];
	if (mgr) {
		dispdev = mgr->device;
	}
	//lcdp=((struct fdisp_lcd_private_data *)dispdev->priv_data);

	//return sprintf(buf, "%d\n", lcdp->bl_need_enabled);
	printk("--get---blen=%d",bl_en);
	return sprintf(buf, "%d\n", bl_en);
}

static ssize_t disp_fbl_enable_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
	int err;
	unsigned long val;
	...
	err = strict_strtoul(buf, 10, &val);
		...
		if(val){
			bl_en=1;
			printk("--set---blen=%d",bl_en);
			gpio_direction_output(lcdp->lcd_cfg.lcd_bl_gpio_hdl, 1);
		}
		else{
			gpio_direction_output(lcdp->lcd_cfg.lcd_bl_gpio_hdl, 0);
			bl_en=0;
			printk("--set-d--blen=%d",bl_en);
		}

	printk("disp_fbl_enable_store val=%d\n",val);
...
static DEVICE_ATTR(fbl_enable, S_IRUGO|S_IWUGO,
    disp_fbl_enable_show, disp_fbl_enable_store);

static ssize_t disp_fb2ui_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
	extern int show_fb_to_ui();
...
static DEVICE_ATTR(fb2ui, S_IRUGO|S_IWUGO,
    disp_fb2ui_show, disp_fb2ui_store);
...
static s32 parser_disp_init_para(const struct device_node *np, disp_init_para * init_para)
{
	...
	if (DISP_SCREEN_NUM > 1) {
	    ...
	    if (of_property_read_u32(np, "lcd0_bright", &value) < 0) {
			__wrn("of_property_read disp_init.lcd0_brightness fail\n");
			init_para->brightness[0]= 50;
		}
		init_para->brightness[0]= value;
	
		if (of_property_read_u32(np, "lcd0_contrast", &value) < 0)	{
			__inf("of_property_read disp_init.lcd0_contrast fail\n");
			init_para->contrast[0]= 50;
		}
		init_para->contrast[0]= value;
	
		if (of_property_read_u32(np, "lcd0_saturation", &value) < 0) {
			__inf("of_property_read lcd0_saturation fail\n");
			init_para->satuation[0]= 50;
		}
		init_para->satuation[0]= value;

		if (of_property_read_u32(np, "lcd0_hue", &value) < 0) {
			__inf("of_property_read lcd0_hue fail\n");
			init_para->hue[0]= 50;
		}
		
		init_para->hue[0]= value;
	
		//fb1
		if (DISP_SCREEN_NUM > 1) {
	
			if (of_property_read_u32(np, "lcd1_bright", &value) < 0) {
				__wrn("of_property_read disp_init.lcd1_brightness fail\n");
				init_para->brightness[1]= 50;
			}
			init_para->brightness[1]= value;
		
			if (of_property_read_u32(np, "lcd1_contrast", &value) < 0)	{
				__inf("of_property_read disp_init.lcd1_contrast fail\n");
				init_para->contrast[1]= 50;
			}
			init_para->contrast[1]= value;
		
			if (of_property_read_u32(np, "lcd1_saturation", &value) < 0) {
				__inf("of_property_read lcd1_saturation fail\n");
				init_para->satuation[1]= 50;
			}
			init_para->satuation[1]= value;

			if (of_property_read_u32(np, "lcd1_hue", &value) < 0) {
				__inf("of_property_read lcd1_hue fail\n");
				init_para->hue[1]= 50;
			}
			
			init_para->hue[1]= value;
		}
#if 0
	//fb2
...
long disp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_ENHANCE_CONTRAST:
	{
		disp_init_para *init_para = &g_disp_drv.disp_init;
		if(ubuffer[0] == 0)
			init_para->contrast[0]=  ubuffer[1];
		else
			init_para->contrast[1]=  ubuffer[1];
		if (enhance && enhance->apply)
			ret = enhance->apply(enhance);
		break;
	}
	case DISP_ENHANCE_SATUATION:
	{
		disp_init_para *init_para = &g_disp_drv.disp_init;
		if(ubuffer[0] == 0)
			init_para->satuation[0]=  ubuffer[1];
		else
			init_para->satuation[1]=  ubuffer[1];
		if (enhance && enhance->apply)
			ret = enhance->apply(enhance);
		break;
	}
