...
static struct ft5x_ts_data *ft5x_ts;

#ifndef CONFIG_INTERRUPT
static struct timer_list timer_in;
#endif
...
static int ft5x_read_data(void)
{
	...
	case 5:
		...
		printk("source data:event->x5 = %d, event->y5 = %d. \n", event->x5, event->y5);
	...
	case 4:
		...
		printk("source data:event->x4 = %d, event->y4 = %d. \n", event->x4, event->y4);
		if(1 == exchange_x_y_flag){
	...
	case 3:
		...
		printk("source data:event->x3 = %d, event->y3 = %d. \n", event->x3, event->y3);
		if(1 == exchange_x_y_flag){
	...
	case 2:
		...
		printk("source data:event->x2 = %d, event->y2 = %d. \n", event->x2, event->y2);
		if(1 == exchange_x_y_flag){
	...
	case 1:
		...
		printk("source data:event->x1 = %d, event->y1 = %d. \n", event->x1, event->y1);
		if(1 == exchange_x_y_flag){
...
static void ft5x_ts_pen_irq_work(struct work_struct *work)
{
	...
	printk("%s:ret:%d\n",__func__,ret);
#ifndef CONFIG_INTERRUPT
	mod_timer(&timer_in, jiffies+HZ/30);
#endif
}
...
#else
void ft5x_ts_timer(unsigned long arg)
{
        printk("==========ft5x_ts timer============\n");
        queue_work(ft5x_ts->ts_workqueue, &ft5x_ts->pen_event_work);
}
#endif
...
static void ft5x_init_events (struct work_struct *work)
{
	...
	printk("read chip_id timers,timers=%d\n",i);
	i = 0;
...
#ifndef CONFIG_INTERRUPT
        init_timer(&timer_in);
        timer_in.function= ft5x_ts_timer;
        add_timer(&timer_in);
        mod_timer(&timer_in, jiffies+HZ/30);
#endif
