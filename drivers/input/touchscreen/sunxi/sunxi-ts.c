...
//#include <mach/lskconfig.h>
//extern NAV_CONFIG_STRU * get_gNavConfig();

//extern int calibrateflag;
//extern int touchevent[3];
//extern wait_queue_head_t calibrate_queue;
//extern int calibration[7];
...
static u32 debug_mask = 0x02;//0x0;
#define dprintk(level_mask, fmt, arg...)	if (unlikely(debug_mask & level_mask)) \
	printk(KERN_DEBUG fmt , ## arg)

//#define printk(level_mask, fmt, arg...)	printk(KERN_DEBUG fmt , ## arg)
...
{
	printk(SUNXITS_PREV"[%s] enter standby state: %d. \n", __FUNCTION__, (int)standby_type);
...
{
	printk(SUNXITS_PREV"[%s] return from standby state: %d. \n", __FUNCTION__, (int)standby_type); 
...
#else
	/* TP_INT_FIFOC: 0x00010313 */
	writel(TP_DATA_XY_CHANGE|TP_DATA_IRQ_EN|TP_FIFO_TRIG_LEVEL|TP_FIFO_FLUSH|TP_UP_IRQ_EN|TP_DOWN_IRQ_EN, TP_BASSADDRESS + TP_INT_FIFOC);
#endif
...
static int calibrate_coord(int *x, int *y)
{
	/*int chy, chx, i;
	chx = (*x) ;
	chy = (*y) ;
	
	(*x) = calibration[0] * chx + calibration[1] * chy + calibration[2];
	(*x) /= calibration[6];

	(*y) = calibration[3] * chx + calibration[4] * chy + calibration[5];
    (*y) /= calibration[6];
	
	printk(SUNXITS_PREV"check-x=%d ,check-y=%d\n", (*x), (*y));*/
	return 0;
}
...
static void report_single_point_implement(struct sunxi_ts_data *ts_data, struct ts_sample_data *sample_data)
{
	...
	input_report_abs(ts_data->input, ABS_MT_POSITION_X, x);
	input_report_abs(ts_data->input, ABS_MT_POSITION_Y, y);   
	
	printk(SUNXITS_PREV"report single point: x = %d, y = %d. \n", x, y);
	    
	input_mt_sync(ts_data->input);                 
	input_sync(ts_data->input);
/*	if(unlikely(calibrateflag)){
		touchevent[0] = x;
		touchevent[1] = y;
		touchevent[2] = 1;
		printk(SUNXITS_PREV"calibrateflag ============wake up===============%d\n", calibrateflag);
		wake_up(&calibrate_queue);
	}
	*/
	return;
}
...
static void report_up_event_implement(struct sunxi_ts_data *ts_data)
{
	...
	if(atomic_sub_and_test(1, &report_up_event_implement_sync)) {
		...
		if (1 == report_up_event_implement_running) {
			...
			printk(SUNXITS_PREV"other thread is running the rountine. \n");
			return;	
		} else {
		...
		printk(SUNXITS_PREV"failed to get the lock. other thread is using the lock. \n");
		return;
	}

	if (0 == data_timer_status) {
		printk(SUNXITS_PREV"report_up_event_implement have been called. \n");
		goto report_up_event_implement_out;
		return;
	}

	printk(SUNXITS_PREV"enter report_up_event_implement. jiffies == %lu. \n", jiffies);
...
static int judge_zoom_orientation(struct ts_sample_data *sample_data)
{
	...
	if (1 == reference_point_flag) {
		...
		printk(SUNXITS_PREV"judge_zoom_orientation: lack reference point .\n");
	}
	       
	printk(SUNXITS_PREV"sunxi-ts: orientation_flag == %d . \n", ret);
	return ret;
}
...
static void change_to_double_mode(struct sunxi_ts_data *ts_data)
{
	...
	(UP_TOUCH_MODE != touch_mode)) {
		printk(SUNXITS_PREV"change_to_double_mode: err, not the expected state. touch_mode = %d. \n", touch_mode);
	}
	...
	if(delta_ds > HOLD_DS_LIMIT){                    /* zoom in */
		...
		if(ZOOM_OUT == zoom_flag) {                    /* zoom in when zoom out */
			if(delta_ds > min(GLIDE_DELTA_DS_MAX_LIMIT, (GLIDE_DELTA_DS_MAX_TIMES*accmulate_zoom_out_ds/zoom_out_count))) {
				...
				printk(SUNXITS_PREV"sunxi-ts: noise, zoom in when zoom out. \n");
				ret = TRUE;
			} else {
				/* normal zoom in */
				zoom_change_cnt++;
				...
				if(zoom_change_cnt > ZOOM_IN_CNT_LIMIT) {
					printk(SUNXITS_PREV"change to ZOOM_IN from ZOOM_OUT. \n");
					change_to_zoom_in(ts_data, sample_data);
					filter_zoom_in_data_init();
					filter_zoom_in_data(&prev_report_samp, sample_data);
				} else {
					printk(SUNXITS_PREV"sunxi-ts: normal zoom in, but this will cause twitter. \n");
					ret = TRUE;
				}
			}
			...
			if(zoom_in_count > (ZOOM_CHANGE_LIMIT_CNT + FIRST_ZOOM_IN_COMPENSTATE)) {
				...
				filter_zoom_in_data_init();
				filter_zoom_in_data(&prev_report_samp, sample_data);
				printk(SUNXITS_PREV"change to ZOOM_IN from ZOOM_INIT_STATE. \n");
				change_to_zoom_in(ts_data, sample_data);
				#if 1
				printk(SUNXITS_PREV"ZOOM_INIT_STATE: delta_ds= %d. \n", delta_ds);
				#endif
			} else	{
				ret = TRUE;
			}
		...
		if(ZOOM_IN == zoom_flag) {                                   /* zoom out when zoom in */
			printk(SUNXITS_PREV"delta_ds = %d, (4*accmulate_zoom_in_ds/zoom_in_count) = %d. \n", \
			...
			if (delta_ds > min(GLIDE_DELTA_DS_MAX_LIMIT, (GLIDE_DELTA_DS_MAX_TIMES*accmulate_zoom_in_ds/zoom_in_count))) {  /* noise */
	 			...
	 			cur_sample_ds = prev_sample_ds;                          /* discard the noise, and can not be reference. */
	 			printk(SUNXITS_PREV"sunxi-ts: noise, zoom out when zoom in. \n");
				...
				if (zoom_change_cnt > ZOOM_OUT_CNT_LIMIT) {
					printk(SUNXITS_PREV"change to ZOOM_OUT from ZOOM_IN. \n");
					change_to_zoom_out(ts_data, sample_data);
					filter_zoom_out_data_init();
					filter_zoom_out_data(&prev_report_samp, sample_data);
				} else {
					printk(SUNXITS_PREV"sunxi-ts: normal zoom out, but this will cause twitter. \n");
					ret = TRUE;
				}
			}
			...
			if (zoom_out_count > ZOOM_CHANGE_LIMIT_CNT) {
				...
				filter_zoom_out_data_init();
				filter_zoom_out_data(&prev_report_samp, sample_data);
				printk(SUNXITS_PREV"change to ZOOM_OUT from ZOOM_INIT_STATE. \n");
				change_to_zoom_out(ts_data, sample_data);
#if 1
				printk(SUNXITS_PREV"ZOOM_INIT_STATE: delta_ds= %d. \n", delta_ds);
#endif
...
static void report_double_point(struct sunxi_ts_data *ts_data, struct ts_sample_data *sample_data)
{
	...
	if(0 == orientation_flag) {
		printk(SUNXITS_PREV"orientation_flag: orientation is not supported or have not known, set the default orientation. \n");
		orientation_flag = ORIENTATION_DEFAULT_VAL;
	}
	...
	printk(SUNXITS_PREV"report two point: x1 = %d, y1 = %d; x2 = %d, y2 = %d. \n",x1, y1, x2, y2);
	printk(SUNXITS_PREV"sample_data->dx = %d, sample_data->dy = %d. \n", sample_data->dx, sample_data->dy);
...
static void report_data(struct sunxi_ts_data *ts_data, struct ts_sample_data *sample_data)
{
	if (TRUE == change_mode) {                                       /* only up event happened, change_mode is allowed. */
		printk(SUNXITS_PREV"err: report_data: never execute. \n ");
		...
		if (ts_data->single_touch_cnt > UP_TO_SINGLE_CNT_LIMIT) { 
			...
			printk(SUNXITS_PREV"change touch mode to SINGLE_TOUCH_MODE from UP state. \n");
		}
		...
		if (ts_data->single_touch_cnt > SINGLE_CNT_LIMIT) {       /* to avoid unconsiously touch */
			...
			change_to_single_touch_mode();
			report_single_point(ts_data, sample_data);            
			printk(SUNXITS_PREV"change touch mode to SINGLE_TOUCH_MODE from double_touch_mode. \n");
		}                                                                              
	}  
	return;
}
...
static void process_data(struct sunxi_ts_data *ts_data, struct ts_sample_data *sample_data)
{
	...
	if (((sample_data->dx) > DUAL_TOUCH)&&((sample_data->dy) > DUAL_TOUCH)) {
		...
		if (UP_TOUCH_MODE == touch_mode ) {
			printk(SUNXITS_PREV"sunxi-ts: need to get the single point. \n");
		...
		if(ts_data->double_point_cnt > DOUBLE_CNT_LIMIT) {
			if(sample_data->dx < MAX_DELTA_X && sample_data->dy < MAX_DELTA_Y) { 
				if(SINGLE_TOUCH_MODE == touch_mode){
					...
					printk(SUNXITS_PREV"sunxi-ts: CHANGING_TO_DOUBLE_TOUCH_MODE orientation_flag == %d . \n", \
						orientation_flag);
					return;
				}
...
///	printk(SUNXITS_PREV"enter tasklet. head = %d, tail = %d. jiffies == %lu. \n", head, tail, jiffies);
	    
	while ((tail) < (head)) {                              /* when tail == head, mean the buffer is empty */
		sample_data = &cycle_buffer[tail&(CYCLE_BUFFER_SIZE-1)];
		tail++;
///		printk(SUNXITS_PREV"sample_data->sample_status == %d, ts_data->ts_process_status == %d \n", \
///			sample_data->sample_status, ts_data->ts_process_status);
		...
		if (TP_UP == sample_data->sample_status || TP_DOWN == sample_data->sample_status) {
			...
			if ((TP_DATA_VA == ts_data->ts_process_status || TP_DOWN == ts_data->ts_process_status) && data_timer_status) {
				...
				printk(SUNXITS_PREV"(prev_sample->sample_time + ts_data->ts_delay_period) == %u, \
					(sample_data->sample_time) == %u. \n", \
					(prev_sample->sample_time + ts_data->ts_delay_period), \
					(sample_data->sample_time));
				printk(SUNXITS_PREV"up or down: sample_data->sample_time = %u.sample_data->sample_status = %d \n", \
				...
				if (time_after_eq((unsigned long)(prev_sample->sample_time + ts_data->ts_delay_period - DELAY_COMPENSTAE_PEROID), \
					...
					printk(SUNXITS_PREV"ignore up event & down event. \n");
					mod_timer(&data_timer, jiffies + ts_data->ts_delay_period);
		...
		case TP_DOWN:
			...
			printk(SUNXITS_PREV"actuall TP_DOWN . \n");
			break;
			
		case TP_DATA_VA:

///			printk(SUNXITS_PREV"data: sample_data->sample_time = %u. \n", sample_data->sample_time);
			...
			if (0 == data_timer_status) {
				...
				printk(SUNXITS_PREV"timer is start up. \n");
			} else {
				mod_timer(&data_timer, jiffies + ts_data->ts_delay_period);
///				printk(SUNXITS_PREV"more ts_data->ts_delay_period ms delay. jiffies + ts_data->ts_delay_period = %lu. \n",\
///					jiffies + ts_data->ts_delay_period);
		...
		case TP_UP :
			...
			if (1 == ts_data->touchflag || 2 == ts_data->touchflag) {
				printk(SUNXITS_PREV"actually TP_UP. \n");
	...
	if (!(reg_val&(TP_DOWN_PENDING | FIFO_DATA_PENDING | TP_UP_PENDING))) {
		...
		if (reg_val&0x40000) {
			...
			if(temp_cnt < (TOTAL_TIMES - 1)) {
				...
				printk(SUNXITS_PREV"temp = ");
				printk(SUNXITS_PREV"%d\n",temp_data);
				temp_data = 0;
				temp_cnt  = 0;
			}

			printk(SUNXITS_PREV"IRQ_HANDLED!\n");	
			return IRQ_HANDLED;
		}
#endif
	    printk(SUNXITS_PREV"IRQ_NONE!\n");	
        return IRQ_NONE;
	}
	/* when head-tail == CYCLE_BUFFER_SIZE, mean the buffer is full. */
	if (((tail+CYCLE_BUFFER_SIZE)) <= (ts_data->buffer_head)) { 
		printk(SUNXITS_PREV"warn: cycle buffer is full. \n");
		...
		printk(SUNXITS_PREV"schedule tasklet. ts_data->buffer_head = %lu, \
...
///		printk(SUNXITS_PREV"press the screen: jiffies to ms == %u , jiffies == %lu, time: = %llu \n", \
...
///		printk(SUNXITS_PREV"data coming, jiffies to ms == %u , jiffies == %lu, time: = %llu \n", \
		jiffies_to_msecs((long)get_jiffies_64()), jiffies, get_cpu_idle_time_us(0, &cur_wall_time));
...
#ifdef TP_INT_PERIOD_TEST
		count++;
		printk(SUNXITS_PREV"jiffies = %d. count = %d. \n", jiffies, count);
#endif
...
///		printk(SUNXITS_PREV"up the screen. jiffies to ms == %u ,  jiffies == %lu,  time: = %llu \n", \
		jiffies_to_msecs((long)get_jiffies_64()), jiffies, get_cpu_idle_time_us(0, &cur_wall_time));
...
static struct sunxi_ts_data *sunxi_ts_data_alloc(struct platform_device *pdev)
{
	...
	input_set_abs_params(ts_data->input, ABS_MT_POSITION_X, 0, 800, 0, 0);
	input_set_abs_params(ts_data->input, ABS_MT_POSITION_Y, 0, 480, 0, 0);

#if 0
	lCarConfig = get_gNavConfig();

	if (FOUR_LINE == lCarConfig->CON1)
	{
//		printk(SUNXITS_PREV"----@@@sun tongyong lCarConfig->tptype = %s @@@---\n",lCarConfig->tptype);
/*	    	if(strstr(lCarConfig->tptype, "RTS") != NULL)
	        {
			printk(SUNXITS_PREV"----@@@sun RTS dianzu @@@---\n");
			ts_data->input->name = "all_key";
	    	}else{
//    		printk(SUNXITS_PREV"----@@@sun  dianrong @@@---\n");
	               ts_data->input->name = "sun_ts";
	        }*/
		ts_data->input->name = "all_key";
	}else
		
	{
//			printk(SUNXITS_PREV"----@@@sun  @@@@@ @@@---\n");
...
static int sunxi_ts_probe(struct platform_device *pdev)
{
	...
	printk(SUNXITS_PREV"sunxi-ts.c: sunxi_ts_probe: start...\n");
	...
	printk(SUNXITS_PREV"begin get platform resourec\n");
...
#ifdef CONFIG_HAS_EARLYSUSPEND	
	printk(SUNXITS_PREV"==register_early_suspend =\n");	
	...
	printk(SUNXITS_PREV"sunxi-ts.c: sunxi_ts_probe: end\n");
...
err_out: 	
	...
	printk(SUNXITS_PREV"sunxi-ts.c: sunxi_ts_probe: failed!\n");
...
static int __init sunxi_ts_init(void)
{
	...
	printk(SUNXITS_PREV"sunxi-ts.c: sunxi_ts_init: start ...\n");
...
static int __init sunxi_ts_init(void)
{
	...
	tp_screen_size = 7;
	tp_regidity_level = 1;
	//tp_press_threshold_enable = 0;
	tp_press_threshold_enable = 0;
	tp_press_threshold = 0x1f40;
	tp_sensitive_level = 0xf;
	tp_exchange_x_y = 0;

	printk(SUNXITS_PREV"tp_screen_size is %d inch.\n", tp_screen_size);
	if (7 == tp_screen_size) {
		dual_touch_distance = 5;
		glide_delta_ds_max_limit = 50;
		tp_regidity_level = 7;
	}else if(5 == tp_screen_size) {
		dual_touch_distance = 35;
		glide_delta_ds_max_limit = 150;
		...
		pr_err(SUNXITS_PREV"tp_screen_size is not supported. \n");
		goto script_get_err;
	}


	if (tp_regidity_level < 2 || tp_regidity_level > 10) {
		printk(SUNXITS_PREV"only tp_regidity_level between 2 and 10  is supported. \n");
		goto script_get_err;
	}

	if(0 != tp_press_threshold_enable  && 1 != tp_press_threshold_enable) {
		printk(SUNXITS_PREV"only tp_press_threshold_enable  0 or 1  is supported. \n");
		goto script_get_err;
	}

	if (1 == tp_press_threshold_enable) {
		printk(SUNXITS_PREV"rtp_press_threshold is %d.\n", tp_press_threshold);
		if(tp_press_threshold < 0 || tp_press_threshold > 0xFFFFFF) {
			printk(SUNXITS_PREV"only tp_regidity_level between 0 and 0xFFFFFF  is supported. \n");
			goto script_get_err;
		}
	}

	printk(SUNXITS_PREV"rtp_sensitive_level is %d.\n", tp_sensitive_level);
	if (tp_sensitive_level < 0 || tp_sensitive_level > 0xf) {
		printk("sunxi-ts: only tp_regidity_level between 0 and 0xf  is supported. \n");
		goto script_get_err;
	}
	...
	printk(SUNXITS_PREV"rtp_exchange_x_y_flag is %d.\n", tp_exchange_x_y);
	if (0 != tp_exchange_x_y && 1 != tp_exchange_x_y) {
		printk(SUNXITS_PREV"only tp_exchange_x_y==1 or  tp_exchange_x_y==0 is supported. \n");
		goto script_get_err;
	}
