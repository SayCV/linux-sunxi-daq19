...
//blusky
        ret = of_property_read_u32(np, "ctp_touch_panel_max_x", &data->touch_panel_max_x);
        if (ret) {
                 pr_err("get ctp_touch_panel_max_x is fail, %d\n", ret);
        }


        ret = of_property_read_u32(np, "ctp_touch_panel_max_y", &data->touch_panel_max_y);
        if (ret) {
                 pr_err("get touch_panel_max_y is fail, %d\n", ret);
        }
