...
//-----------------------------------------------------------------LANDSEM@yingxianFei add-----------

/*
 * On most platforms, we'd rather do straight i2c I/O.
 */

int cci_read_a8_d8_passby(struct v4l2_subdev *sd, unsigned char slave,unsigned char addr,
    unsigned char *value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
  ...
  if (ret >= 0) {
    *value = data[1];
    ...
    cci_err("%s error! slave = 0x%x, addr = 0x%2x, value = 0x%2x\n ",__func__, slave, addr,*value);
  }
  return ret;
#endif
}
EXPORT_SYMBOL_GPL(cci_read_a8_d8_passby);

int cci_write_a8_d8_passby(struct v4l2_subdev *sd, unsigned char slave,unsigned char addr,
    unsigned char value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
...
EXPORT_SYMBOL_GPL(cci_write_a8_d8_passby);

int cci_read_a8_d16_passby(struct v4l2_subdev *sd, unsigned char slave,unsigned char addr,
    unsigned short *value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
...
EXPORT_SYMBOL_GPL(cci_read_a8_d16_passby);

int cci_write_a8_d16_passby(struct v4l2_subdev *sd, unsigned char slave,unsigned char addr,
    unsigned short value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
...
EXPORT_SYMBOL_GPL(cci_write_a8_d16_passby);
int cci_read_a16_d8_passby(struct v4l2_subdev *sd, unsigned char slave,unsigned short addr,
    unsigned char *value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
...
EXPORT_SYMBOL_GPL(cci_read_a16_d8_passby);

int cci_write_a16_d8_passby(struct v4l2_subdev *sd, unsigned char slave,unsigned short addr,
    unsigned char value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
...
EXPORT_SYMBOL_GPL(cci_write_a16_d8_passby);

int cci_read_a16_d16_passby(struct v4l2_subdev *sd, unsigned char slave, unsigned short addr,
    unsigned short *value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
...
EXPORT_SYMBOL_GPL(cci_read_a16_d16_passby);

int cci_write_a16_d16_passby(struct v4l2_subdev *sd, unsigned char slave, unsigned short addr,
    unsigned short value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
...
EXPORT_SYMBOL_GPL(cci_write_a16_d16_passby);

int cci_read_a0_d16_passby(struct v4l2_subdev *sd, unsigned char slave, unsigned short *value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
	return cci_rd_0_16(cci_drv->cci_id, value, cci_drv->cci_saddr);
  ...
  if (ret >= 0) {
	*value = data[0]*256 + data[1];
    ...
    cci_err("%s error! slave = 0x%x, value = 0x%4x\n ",__func__, slave, *value);
  }
  return ret;
#endif
}

EXPORT_SYMBOL_GPL(cci_read_a0_d16_passby);

int cci_write_a0_d16_passby(struct v4l2_subdev *sd, unsigned char slave, unsigned short value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
	return cci_wr_0_16(cci_drv->cci_id, value, cci_drv->cci_saddr);
...
EXPORT_SYMBOL_GPL(cci_write_a0_d16_passby);

int cci_write_passby(struct v4l2_subdev *sd, unsigned char slave,addr_type addr, data_type value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
#else
	...
	if(8 == addr_width && 8 == data_width)
	{
		return cci_write_a8_d8_passby(sd, slave,(unsigned char)addr, (unsigned char)value);
	}
	else if (8 == addr_width && 16 == data_width)
	{
		return cci_write_a8_d16_passby(sd, slave,(unsigned char)addr, value);
	}
	else if (16 == addr_width && 8 == data_width)
	{
		return cci_write_a16_d8_passby(sd, slave,addr, (unsigned char)value);
	}
	else if (16 == addr_width && 16 == data_width)
	{
		return cci_write_a16_d16_passby(sd, slave,addr, value);
	}
	else if (0 == addr_width && 16 == data_width)
	{
		return cci_write_a0_d16_passby(sd, slave,value);
	}
	else
	{
		cci_err("%s error! addr_width = %d , data_width = %d!\n ",__func__, addr_width, data_width);
...
EXPORT_SYMBOL_GPL(cci_write_passby);


int sensor_write_passby(struct v4l2_subdev *sd, unsigned char slave,addr_type reg, data_type value)
{
	int ret=0, cnt=0;
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
	...
	ret = cci_write_passby(sd,slave,reg,value);
	while((ret != 0) && (cnt < 2))
	{
		ret = cci_write_passby(sd,slave,reg,value);
		cnt++;
...
EXPORT_SYMBOL_GPL(sensor_write_passby);


/*
 * Write a list of register settings;
 */
int sensor_write_array_passby(struct v4l2_subdev *sd,unsigned char slave, struct regval_list *regs, int array_size)
{
	int ret = 0, i = 0;
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
	...
	while(i < array_size)
	{
		if(regs->addr == REG_DLY) {
			...
			ret = sensor_write_passby(sd,slave,regs->addr, regs->data);
			if(ret < 0)
				printk("%s sensor write array error!!\n",cci_drv->name);
		}
		i++;
...
EXPORT_SYMBOL_GPL(sensor_write_array_passby);

int cci_read_passby(struct v4l2_subdev *sd, unsigned char slave,addr_type addr, data_type *value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
#else
	...
	*value = 0;
	if(8 == addr_width && 8 == data_width)
	{
		return cci_read_a8_d8_passby(sd, slave,(unsigned char)addr, (unsigned char *)value);
	}
	else if (8 == addr_width && 16 == data_width)
	{
		return cci_read_a8_d16_passby(sd, slave,(unsigned char)addr, value);
	}
	else if (16 == addr_width && 8 == data_width)
	{
		return cci_read_a16_d8_passby(sd, slave,addr, (unsigned char *)value);
	}
	else if (16 == addr_width && 16 == data_width)
	{
		return cci_read_a16_d16_passby(sd, slave,addr, value);
	}
	else if (0 == addr_width && 16 == data_width)
	{
		return cci_read_a0_d16_passby(sd, slave,value);
	}
	else
	{
		cci_err("%s error! addr_width = %d , data_width = %d!\n ",__func__, addr_width, data_width);
...
EXPORT_SYMBOL_GPL(cci_read_passby);

/*
 * On most platforms, we'd rather do straight i2c I/O.
 */
int sensor_read_passby(struct v4l2_subdev *sd, unsigned char slave,addr_type reg, data_type *value)
{
	int ret=0, cnt=0;
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);
	...
	ret = cci_read_passby(sd,slave,reg,value);
	while((ret != 0) && (cnt < 2))
	{
		ret = cci_read_passby(sd,slave,reg,value);
		cnt++;
...
EXPORT_SYMBOL_GPL(sensor_read_passby);
