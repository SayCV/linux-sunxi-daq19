			 ...
			 struct car_reverse_private_data *priv)
{
	...
	of_get_value_by_name(pdev,
		"vfe_id", &priv->config.vfe_id, 0);
		
	of_get_value_by_name(pdev,
		"input_sync", &priv->config.input_sync, 0);
	...
	of_get_gpio_by_name(pdev,
		"reverse_pin", &priv->config.gpio_reverse_pin);

	if(priv->config.input_sync == INPUT_CSI)
		{
			of_get_value_by_name(pdev,
		"src_width", &priv->config.src_width, 0);
			of_get_value_by_name(pdev,
		"src_height", &priv->config.src_height, 0);
		}
}
...
void car_reverse_display_update(void)
{
	...
	while (!list_empty(pending_frame)) {
		...
		if(car_reverse->config.input_sync == INPUT_CSI)
			video_source_queue_buffer(node,INPUT_CSI);
		else if(car_reverse->config.input_sync == INPUT_TVD)
			video_source_queue_buffer(node,INPUT_TVD);
	}
	//node = video_source_dequeue_buffer();
	if(car_reverse->config.input_sync == INPUT_CSI)
		node = vfe_video_source_dequeue_buffer();
	else if(car_reverse->config.input_sync == INPUT_TVD)
		node = tvd_video_source_dequeue_buffer();
...
static int car_reverse_preview_start(void)
{
	...
	for (i = 0; i < (SWAP_BUFFER_CNT - 1); i++) {
		...
		if(car_reverse->config.input_sync == INPUT_TVD)
			video_source_queue_buffer(node,INPUT_TVD);
		
		else if(car_reverse->config.input_sync == INPUT_CSI)
			video_source_queue_buffer(node,INPUT_CSI);
	}

	//video_source_streamon();
	video_source_streamon(car_reverse->config.input_sync);
	
	car_reverse->status = CAR_REVERSE_START;
	car_reverse->thread_mask = 0;
	car_reverse->first_stmp=1;
	return 0;
}
...
static int car_reverse_preview_stop(void)
{
	...
	video_source_streamoff(car_reverse->config.input_sync);
...
static int display_update_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		spin_unlock(&car_reverse->display_lock);
			...
			if(unlikely(car_reverse->first_stmp==1)){
			printk(KERN_EMERG"start car reverse disp first frame,\n"); 
			
			
			car_reverse->first_stmp=0;
		}
...
static struct class_attribute car_reverse_attrs[] = {
	...
	__ATTR(needexit, S_IRWXUGO,
			car_reverse_needexit_show, car_reverse_needexit_store),
#ifdef _REVERSE_DEBUG_
...
static int car_reverse_probe(struct platform_device *pdev)
{
	...
	if(car_reverse->config.input_sync == INPUT_CSI)
		car_reverse->buffer_pool = alloc_buffer_pool(&pdev->dev, SWAP_BUFFER_CNT, 1280*720*2);
	else if(car_reverse->config.input_sync == INPUT_TVD)	
  	    car_reverse->buffer_pool = alloc_buffer_pool(&pdev->dev, SWAP_BUFFER_CNT, 720*576*2);  //INPUT_TVD
...
//fs_initcall(car_reverse_module_init);
fs_initcall_sync(car_reverse_module_init);
