...
static volatile int tvd_onsingal;




#if defined(CONFIG_SWITCH) || defined(CONFIG_ANDROID_SWITCH)
static struct task_struct * g_tvd_task;
static u8 g_tvd_hpd[4]={1};
static u8 tvd_number=0;
static struct switch_dev tvd_switch_dev = {
	.name = "tvd_signal",
};
#endif
static unsigned char b_tvd_suspend = 0;
...
static ssize_t tvd0_signal_dump_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{

		unsigned long val;

		val=tvd_get_signal(0); 
		if (val==0x00000000)
			{
			 switch_set_state(&tvd_switch_dev, 1);	
			 return sprintf(buf, "0x%08x\n", val);
		}
		else{
			 switch_set_state(&tvd_switch_dev, 0);
			 return sprintf(buf, "0x%08x\n", val);
			}
}

static ssize_t tvd1_signal_dump_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{

		unsigned long val;

		val=tvd_get_signal(1); 
		if (val==0x00000000)
			{
			 switch_set_state(&tvd_switch_dev, 1);	
			 return sprintf(buf, "0x%08x\n", val);
...
static ssize_t tvd2_signal_dump_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{

		unsigned long val;

		val=tvd_get_signal(2); 
		if (val==0x00000000)
			{
			 switch_set_state(&tvd_switch_dev, 1);	
			 return sprintf(buf, "0x%08x\n", val);
...
static ssize_t tvd3_signal_dump_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
		unsigned long val;
		val=tvd_get_signal(3); 
		if (val==0x00000000)
			{
			 switch_set_state(&tvd_switch_dev, 1);	
			 return sprintf(buf, "0x%08x\n", val);
...
static ssize_t tvd0_signal_dump_store(struct device *dev,
		    struct device_attribute *attr,
		    const char *buf, size_t count)
{
	int err;
...
static ssize_t tvd1_signal_dump_store(struct device *dev,
		    struct device_attribute *attr,
		    const char *buf, size_t count)
{
	int err;
...
static ssize_t tvd2_signal_dump_store(struct device *dev,
		    struct device_attribute *attr,
		    const char *buf, size_t count)
{
	int err;
...
static ssize_t tvd3_signal_dump_store(struct device *dev,
		    struct device_attribute *attr,
		    const char *buf, size_t count)
{
	int err;
...
#ifdef CONFIG_AW_AXP_CONFIG
static int TVD_power_enable(void)
{

	struct regulator *regu= NULL;
	int ret = -1;
	regu= regulator_get(NULL, "vcc-tvin");
	if (IS_ERR(regu)) {
	...
	if (0 != ret) {
		...
		printk("suceess to enable regulator %s!\n", "vcc-tvin");
	}
...
static int TVD_power_disable(void)
{
	struct regulator *regu= NULL;
	int ret = 0;
	regu= regulator_get(NULL, "vcc-tvin");
	...
	if (0 != ret) {
		...
		printk("suceess to disable regulator %s!\n", "vcc-tvin");
	}
...
static int TVD_put_regulator(void)
{
	struct regulator *regu= NULL;
	int ret = 0;
	regu= regulator_get(NULL, "vcc-tvin");
	if (IS_ERR(regu)) {
		pr_err("%s: TVD_put_regulator happen, fail to get regulator %s\n", __func__, "vcc-tvin");
		goto exit;
	}
     regulator_put(regu);
exit:	 
...
#else
static int TVD_power_enable(void){return 0;}
static int TVD_power_disable(void){return 0;}
static int TVD_put_regulator(void){return 0;}
...
s32 tvd_detect_thread(void *parg)
{
	u32 hpd[4]={1};
	u32 tep_tvd;
	u32 i = 0,j=0;
	...
	while(1) {
		...
		for(i=0;i<4;i++)
			{
		hpd[i]=tvd_get_signal(i);
		if((hpd[i] != g_tvd_hpd[i])){
			...
			if((hpd[i] != g_tvd_hpd[i]))
              {
                g_tvd_hpd[i]=hpd[i];
				num=((i+4)<<8)|((~hpd[i])&0x0001);
				id=num>>8;
				printk("hpd :tvd_num[] = %x,g_status[] =%x,tvd_onsingal=%x\n",id,num,tvd_onsingal);	
				switch_set_state(&tvd_switch_dev,((i+4)<<8)|((~hpd[i])&0x0001));	
               }		
	 		}
		 }
...
static DEVICE_ATTR(tvd0_signal, S_IRUGO|S_IWUSR|S_IWGRP,
		tvd0_signal_dump_show, tvd0_signal_dump_store);
static DEVICE_ATTR(tvd1_signal, S_IRUGO|S_IWUSR|S_IWGRP,
		tvd1_signal_dump_show, tvd1_signal_dump_store);
static DEVICE_ATTR(tvd2_signal, S_IRUGO|S_IWUSR|S_IWGRP,
		tvd2_signal_dump_show, tvd2_signal_dump_store);
static DEVICE_ATTR(tvd3_signal, S_IRUGO|S_IWUSR|S_IWGRP,
		tvd3_signal_dump_show, tvd3_signal_dump_store);
...
static int __tvd_3d_comp_mem_request(struct tvd_dev *dev, int size)
{
...
#ifdef TVD_RESERVED_MEM/*if use cma defined TVD_RESERVED_MEM*/
		unsigned map_size = 0;
		struct page *page;
		map_size = PAGE_ALIGN(size);
		page = alloc_pages(GFP_KERNEL,get_order(map_size));
		if (page != NULL) {
			dev->fliter.vir_address = page_address(page);
			if (NULL == dev->fliter.vir_address) {
				free_pages((unsigned long)(page),get_order(map_size));
				printk(KERN_ERR "page_address fail!\n");
				return -ENOMEM;
			}
			dev->fliter.p_addr = virt_to_phys(dev->fliter.vir_address);
			dev->fliter.phy_address = (void *)dev->fliter.p_addr;
			pr_debug("pa=0x%p va=0x%p size:0x%x\n",(void*)dev->fliter.p_addr , dev->fliter.vir_address, size);
			memset(dev->fliter.vir_address,0,size);
			return 0;
		}	else {
	...
	pr_debug("__tvd_3d_comp_mem_request_size=%x\n",dev->fliter.size);
	dev->fliter.vir_address = dma_alloc_coherent(dev->v4l2_dev.dev, dev->fliter.size ,
...
{
#ifdef TVD_RESERVED_MEM
	unsigned map_size = PAGE_ALIGN(0x200000);
	unsigned page_size = map_size;
	if (NULL == dev->fliter.vir_address) {
		printk(KERN_ERR "%s: failed!\n", __func__);
		return -1;
	}
	free_pages((unsigned long)(dev->fliter.vir_address),get_order(page_size));
	dev->fliter.vir_address = NULL;
	pr_debug("mem_free::va=0x%p size:0x%x\n", dev->fliter.vir_address, page_size);
#else
	   u32 actual_bytes;
...
#ifdef tvd_check
	if (0 == tvd_valid_frame_setting(dev->sel))
	{
		goto unlock;
	}
#endif	
...
{
	int i = 0;
	pr_debug("get_status:dev->interface = %d\n", dev->interface);
...
static int vidioc_g_fmt_vid_cap(struct file *file, void *priv,struct v4l2_format *f)
{
	...
	f->fmt.pix.width = 720;
    f->fmt.pix.height = 480;
	pr_debug("%s :tvd detect ypbprp  system = %d, w = %d, h = %d\n",
	__func__, system, f->fmt.pix.width, f->fmt.pix.height);
	return 0;
 }
 else if(dev->interface==1)
 	{
	f->fmt.pix.width = 720;
    f->fmt.pix.height = 576;
	pr_debug("%s :tvd detect ypbprI system = %d, w = %d, h = %d\n",
	__func__, system, f->fmt.pix.width, f->fmt.pix.height);
	return 0;
 	}
 else{
  pr_debug("tvd detect no signal\n");}
	}
	}
	pr_debug("%s : system = %d, w = %d, h = %d\n",__func__, system, f->fmt.pix.width, f->fmt.pix.height);
	return 0;
	}
}
...
int tvd_open_special(int tvd_fd)
{
	...
	ret = __tvd_clk_enable(dev);
	__tvd_init(dev);
	tvd_init(dev->sel, dev->interface);/*add for tvd_init adc by david*/
	dev->input = 0;
...
int vidioc_g_fmt_vid_cap_special(int tvd_fd, struct v4l2_format *f)
{
	...
	if (!locked) {
		pr_err("%s: signal is not locked.\n", __func__);
		pr_err("%s: signal is not locked add by wxh .\n", __func__);
		return -EAGAIN;
	} else {
		/* system: 1->pal, 0->ntsc   signal 0 1*/
		if (system == PAL) {
			f->fmt.pix.width = 720;
			f->fmt.pix.height = 576;
		} else if (system == NTSC) {
...
static int __tvd_init_controls(struct v4l2_ctrl_handler *hdl)
{
	...
	//v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_BRIGHTNESS, -1, 255, 1, 0x20);
	//v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_CONTRAST, -1, 255, 1, 0x80);
	//v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_SATURATION, -1, 255, 1, 0x80);
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_BRIGHTNESS, -1, 100, 1, 32);
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_CONTRAST, -1, 255, 1, 128);
	v4l2_ctrl_new_std(hdl, &tvd_ctrl_ops, V4L2_CID_SATURATION, -1, 255, 1, 128);
...
static int tvd_probe(struct platform_device *pdev)
{
	...
	#if defined(CONFIG_SWITCH) || defined(CONFIG_ANDROID_SWITCH)
	switch_dev_register(&tvd_switch_dev);
	g_tvd_task = kthread_create(tvd_detect_thread, (void*)0, "tvd-detect");
	if(IS_ERR(g_tvd_task))
    {
        __s32 err = 0;
        err = PTR_ERR(g_tvd_task);
        g_tvd_task = NULL;
        return err;
    }
    wake_up_process(g_tvd_task);
	#endif
