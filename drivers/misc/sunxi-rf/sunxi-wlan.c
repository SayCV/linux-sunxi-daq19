...
/*******************************************************************/
	...
	if (!gpio_is_valid(clk_gpio)) {
		...
		int ls_uboot_power_on = of_get_named_gpio_flags(np, "ls_uboot_power_on", 0, (enum of_gpio_flags *)&config);
	if (!gpio_is_valid(ls_uboot_power_on)) {
		printk( "get gpio ls_uboot_power_on failed\n");
	} else {
		printk( "ls_uboot_power_on gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",
				config.gpio,
				config.mul_sel,
				config.pull,
				config.drv_level,
		...
		ret = devm_gpio_request(dev, ls_uboot_power_on,
				"ls_uboot_power_on");
		if (ret < 0) {
			printk( "can't request ls_uboot_power_on gpio %d\n",
				ls_uboot_power_on);
			return ret;
		}

		ret = gpio_direction_output(ls_uboot_power_on, 1);
		if (ret < 0) {
			printk( "can't request output direction ls_uboot_power_on gpio %d\n",
				ls_uboot_power_on);
			return ret;
		}
	}
			int ls_uboot_lcd_power = of_get_named_gpio_flags(np, "ls_uboot_lcd_power", 0, (enum of_gpio_flags *)&config);
	if (!gpio_is_valid(ls_uboot_lcd_power)) {
		printk( "get gpio ls_uboot_lcd_power failed\n");
	} else {
		printk( "ls_uboot_lcd_power gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",
				config.gpio,
				config.mul_sel,
				config.pull,
				config.drv_level,
		...
		ret = devm_gpio_request(dev, ls_uboot_lcd_power,
				"ls_uboot_lcd_power");
		if (ret < 0) {
			printk( "can't request ls_uboot_lcd_power gpio %d\n",
				ls_uboot_lcd_power);
			return ret;
		}

		ret = gpio_direction_output(ls_uboot_lcd_power, 1);
		if (ret < 0) {
			printk( "can't request output direction ls_uboot_lcd_power gpio %d\n",
				ls_uboot_lcd_power);
			return ret;
		}
	}
			int ls_uboot_blk_power = of_get_named_gpio_flags(np, "ls_uboot_blk_power", 0, (enum of_gpio_flags *)&config);
	if (!gpio_is_valid(ls_uboot_blk_power)) {
		printk( "get gpio ls_uboot_blk_power failed\n");
	} else {
		printk( "ls_uboot_blk_power gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",
				config.gpio,
				config.mul_sel,
				config.pull,
				config.drv_level,
		...
		ret = devm_gpio_request(dev, ls_uboot_blk_power,
				"ls_uboot_blk_power");
		if (ret < 0) {
			printk( "can't request ls_uboot_blk_power gpio %d\n",
				ls_uboot_blk_power);
			return ret;
		}

		ret = gpio_direction_output(ls_uboot_blk_power, 1);
		if (ret < 0) {
			printk( "can't request output direction ls_uboot_blk_power gpio %d\n",
				ls_uboot_blk_power);
			return ret;
		}
	}
			int ls_uboot_lcd_bl_en = of_get_named_gpio_flags(np, "ls_uboot_lcd_bl_en", 0, (enum of_gpio_flags *)&config);
	if (!gpio_is_valid(ls_uboot_lcd_bl_en)) {
		printk( "get gpio ls_uboot_lcd_bl_en failed\n");
	} else {
		printk( "ls_uboot_lcd_bl_en gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",
				config.gpio,
				config.mul_sel,
				config.pull,
				config.drv_level,
		...
		ret = devm_gpio_request(dev, ls_uboot_lcd_bl_en,
				"ls_uboot_lcd_bl_en");
		if (ret < 0) {
			printk( "can't request ls_uboot_lcd_bl_en gpio %d\n",
				ls_uboot_lcd_bl_en);
			return ret;
		}

		ret = gpio_direction_output(ls_uboot_lcd_bl_en, 1);
		if (ret < 0) {
			printk( "can't request output direction ls_uboot_lcd_bl_en gpio %d\n",
				ls_uboot_lcd_bl_en);
			return ret;
		}
	}
