...
static long sunxi_di_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (!mutex_trylock(&di_data->slock)) {
			mutex_lock(&di_data->slock);
	}
	switch (cmd) {
	case DI_IOCSTART:
		{
