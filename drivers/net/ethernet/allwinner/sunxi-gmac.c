...
static int geth_power_on(struct geth_priv *priv)
{
	...
	if (gpio_is_valid(priv->phyenable))
	{
		gpio_direction_output(priv->phyenable, 1);
	}
...
static void geth_power_off(struct geth_priv *priv)
{
	...
	if (gpio_is_valid(priv->phyenable))
	{
		gpio_direction_output(priv->phyenable, 0);
	}
...
static int geth_script_parse(struct platform_device *pdev)
{
	...
	priv->phyenable = of_get_named_gpio_flags(np, "phy-enable", 0,
						(enum of_gpio_flags *)&cfg);
...
static int geth_sys_request(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(priv->phyenable)) {
		ret = gpio_request(priv->phyenable, "phy-rst");
		if (ret < 0)
			goto pin_err;
		gpio_direction_output(priv->phyenable, 1);
		printk(KERN_ERR "init power enable!!\n");
	}
